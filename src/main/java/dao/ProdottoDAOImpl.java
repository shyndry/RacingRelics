...
public class ProdottoDAOImpl implements ProdottoDAO {
    ...
    public void doSave(Prodotto prodotto) throws SQLException{
        ...
        try (Connection con = ConnessioneDB.getConnection();
            ...
            ps.setDouble(3, prodotto.getPrezzoAttuale());
            ps.setInt(4, prodotto.getQuantitaDisponibile());
            ps.setString(5, prodotto.getImmaginePath());
            ps.setString(6, prodotto.getScuderia());
            ps.setString(7, prodotto.getPilota());
            ps.setInt(8, prodotto.getAnnoCampionato());
        ...
        try (Connection con = ConnessioneDB.getConnection();
            PreparedStatement ps = con.prepareStatement(query)){
                ...
                try(ResultSet rs = ps.executeQuery()) {
                    if (rs.next()){
                        prodotto = estraiProdottoDaResultSet(rs);
                    }
                }
        }
        return prodotto;
    ...
    private Prodotto estraiProdottoDaResultSet(ResultSet rs) throws SQLException{
        ...
        p.setImmaginePath(rs.getString("immagine_path"));
        p.setScuderia(rs.getString("scuderia"));
        p.setPilota(rs.getString("pilota"));
        p.setAnnoCampionato(rs.getInt("anno_campionato"));
