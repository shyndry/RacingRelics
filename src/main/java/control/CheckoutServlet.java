    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection con = ConnessioneDB.getConnection()) {
                ...
                try (PreparedStatement psStock = con.prepareStatement(updateStockQuery)) {
                    ...
                    for (Map.Entry<Integer, Integer> entry : carrello.entrySet()) {
                        ...
                        if (prodotto != null && prodotto.isAttivo()
                                && prodotto.getQuantitaDisponibile() >= quantitaRichiesta) {

                            ordineDAO.doSaveComposizione(idOrdineGenerato, idProdotto, quantitaRichiesta,
                                    prodotto.getPrezzo(), con);
                            ...
                            throw new SQLException(
                                    "Scorte insufficienti o reperto non più attivo per l'ID prodotto: " + idProdotto);
                        }
                    }
                }
            ...
            request.setAttribute("errorMessage",
                    "Transazione annullata. Verificare la disponibilità dei reperti selezionati.");
            request.getRequestDispatcher("/WEB-INF/views/common/carrello.jsp").forward(request, response);
        }
    }
}